...
export default function TeacherDashboard() {
  ...
  const courses = useMemo(() => {
    const names = (Array.isArray(resultsData) ? resultsData : [])
      .map((r) => r?.course?.courseName)
      .filter(Boolean);
    return ['All Courses', ...Array.from(new Set(names))];
  }, [resultsData]);

  const filteredResults = useMemo(() => {
    const list = Array.isArray(resultsData) ? resultsData : [];

    return list
      .filter((r) => !selectedProgram || r?.student?.program === selectedProgram)
      .filter(
        (r) =>
          !selectedSemester ||
          String(r?.student?.semester ?? '') === String(selectedSemester)
      )
      .filter((r) => !selectedCourse || r?.course?.courseName === selectedCourse)
      .filter((r) => {
        if (!searchTerm) return true;
        const name = (r?.student?.fullName || '').toLowerCase();
        return name.includes(searchTerm.toLowerCase());
      });
  }, [resultsData, selectedProgram, selectedSemester, selectedCourse, searchTerm]);
  ...
  async function fetchResults() {
    ...
    try {
      const res = await axios.get(`${API_BASE_URL}/result`, getAxiosConfig());
      if (res.data?.success) setResultsData(res.data.data || []);
      else setResultsData([]);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (hash === 'courses-exams') setActiveTab('Courses/Exams');
    else setActiveTab('Dashboard');
  }, []);

  useEffect(() => {
    if (selectedPool) fetchPoolQuestions(selectedPool, selectedDifficulty);
  }, [selectedPool, selectedDifficulty]);

  const handleSidebarNav = (label) => {
    if (label !== 'Results') setActiveTab(label);
  };

  const fetchCourses = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/courses`, getAxiosConfig());
      if (response.data.success) setCoursesData(response.data.data);
    } catch (err) {
  ...
  const fetchQuestionPools = async () => {
    try {
      ...
      if (response.data.success) setQuestionPools(response.data.data);
    } catch (err) {
  ...
  const fetchPoolQuestions = async (poolId, difficulty = 'all') => {
    try {
      ...
      if (response.data.success) setPoolQuestions(response.data.data);
    } catch (err) {
  ...
  const handleAddQuestion = async (e) => {
    ...
    try {
      const options = [
        { optionText: newQuestion.option1, isCorrect: parseInt(newQuestion.correctAnswer, 10) === 1 },
        { optionText: newQuestion.option2, isCorrect: parseInt(newQuestion.correctAnswer, 10) === 2 },
        { optionText: newQuestion.option3, isCorrect: parseInt(newQuestion.correctAnswer, 10) === 3 },
        { optionText: newQuestion.option4, isCorrect: parseInt(newQuestion.correctAnswer, 10) === 4 }
      ];
      ...
      const response = await axios.post(
        `${API_BASE_URL}/questions`,
        {
          ...
          options,
          correctAnswer: parseInt(newQuestion.correctAnswer, 10),
  ...
  const handleDeleteQuestion = async (questionId) => {
    ...
    try {
      const response = await axios.delete(`${API_BASE_URL}/questions/${questionId}`, getAxiosConfig());
  ...
  const handleDeleteCourse = async (courseId) => {
    ...
    try {
      const response = await axios.delete(`${API_BASE_URL}/courses/${courseId}`, getAxiosConfig());
        ...
        return (
          <div className="p-4">
            ...
            <div className="row g-4 mb-5 stat-card-row">
              ...
              <div className="col-md-4">
                <div className="card border-0 shadow-sm stat-card">
                  <div className="card-body text-center">
                    <div className="mb-2">
                      <span className="text-success" style={{ fontSize: '32px' }}>
                        📝
                      </span>
                    </div>
            ...
            <div className="mt-5">
              ...
              <div className="alert alert-info">Showing activity related to course and pool creation...</div>
            </div>
          </div>
        );
          ...
          <div className="p-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
              ...
              <div className="d-flex flex-wrap gap-2">
                ...
                <button className="btn btn-primary" onClick={() => setShowCourseModal(true)} disabled={loading}>
                  + Add Course
                </button>
              </div>
            </div>
            ...
            <div className="mb-5">
              ...
              {questionPools.length === 0 ? (
                <div className="alert alert-info">No question pools yet. Create one to start adding questions!</div>
              ) : (
                            ...
                            <div className="mb-3">
                              <div className="d-flex justify-content-between mb-1">
                                <small className="text-success">
                                  Easy: {stats.easy}/{totalEasyTarget}
                                </small>
                              ...
                              <div className="progress mb-2 rounded-pill" style={{ height: '6px' }}>
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, (stats.easy / totalEasyTarget) * 100)}%` }}
                                ></div>
                              </div>

                              <div className="d-flex justify-content-between mb-1">
                                <small className="text-warning">
                                  Medium: {stats.medium}/{totalMediumTarget}
                                </small>
                              ...
                              <div className="progress mb-2 rounded-pill" style={{ height: '6px' }}>
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, (stats.medium / totalMediumTarget) * 100)}%` }}
                                ></div>
                              </div>

                              <div className="d-flex justify-content-between mb-1">
                                <small className="text-danger">
                                  Hard: {stats.hard}/{totalHardTarget}
                                </small>
                              ...
                              <div className="progress mb-2 rounded-pill" style={{ height: '6px' }}>
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, (stats.hard / totalHardTarget) * 100)}%` }}
                                ></div>
                              </div>
                            </div>
                ...
                <div className="card-body p-4">
                  <div className="d-flex justify-content-between align-items-center mb-4">
                    <h5 className="fw-bold text-dark mb-0">
                      Questions in {questionPools.find((p) => p._id === selectedPool)?.poolName}
                    </h5>
                    <div>
                      <select
                      ...
                      <div className="card border-0 shadow-sm h-100 course-card">
                        <div className="card-body d-flex flex-column">
                          ...
                          <div className="d-grid gap-2 mt-auto">
                            <button className="btn btn-primary btn-sm" onClick={() => handleAssignQuiz(course)}>
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="16"
                                height="16"
                            ...
                            <div className="d-flex gap-2">
                              <button className="btn btn-success btn-sm flex-grow-1">Manage</button>
                              <button
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <h3 className="fw-bold mb-0">
                          {filteredResults.length > 0
                            ? Math.max(...filteredResults.map((r) => Number(r?.percentage || 0))).toFixed(1)
                            : 0}
                          %
                        </h3>
                      </div>
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        <h3 className="fw-bold mb-0">
                          {filteredResults.length > 0
                            ? (
                                (filteredResults.filter((r) => Number(r?.percentage || 0) >= 60).length /
                                  filteredResults.length) *
                                100
                              ).toFixed(1)
                            : 0}
      ...
      default:
        return (
          <div className="p-4 text-center">
            <div className="alert alert-warning mt-5">Select a menu item from the sidebar to view content.</div>
          </div>
        );
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <style>
        {`
          body { background-color: #f7f9fc; overflow-x: hidden; }
          .card { transition: transform 0.3s ease, box-shadow 0.3s ease; border-radius: 12px; border: none; }
          ...
          .sidebar-nav-btn { transition: background-color 0.2s, color 0.2s, box-shadow 0.3s; border-radius: 8px !important; }
          .sidebar-nav-btn:hover:not([style*="background-color: rgb(25, 135, 84)"]) { background-color: rgba(255, 255, 255, 0.05) !important; }
          .logout-btn { color: #dc3545 !important; background-color: transparent !important; transition: all 0.3s ease; }
          .logout-btn:hover { background-color: #dc3545 !important; color: white !important; }
          .logout-btn:hover .logout-svg { stroke: white !important; }
          .logout-svg { stroke: #dc3545; transition: stroke 0.3s ease; }
          .main-content-wrapper { transition: margin-left 0.3s ease-in-out; margin-left: 280px; min-height: 100vh; width: calc(100% - 280px); overflow-y: auto; overflow-x: hidden; }
          .results-table-container { border: 1px solid #e9ecef; border-radius: 8px; overflow-x: auto; }

          @media (max-width: 991.98px) {
            .sidebar-container { position: fixed; transform: translateX(-280px); box-shadow: 2px 0 5px rgba(0, 0, 0, 0.3); width: 280px !important; height: 100vh; left: 0; top: 0; }
            .sidebar-container.active { transform: translateX(0); }
            .main-content-wrapper { margin-left: 0 !important; width: 100%; }
            .mobile-header { position: sticky; top: 0; z-index: 1000; background-color: #ffffff; border-bottom: 1px solid #e0e0e0; padding: 1rem; display: flex; justify-content: space-between; align-items: center; }
            ...
            .stat-card-row > div { flex: 0 0 100%; max-width: 100%; margin-bottom: 0.75rem !important; }
            .d-flex.justify-content-between.align-items-center { flex-direction: column; align-items: flex-start !important; gap: 1rem; }
          }
        `}
      </style>
              ...
              <div className="modal-body">
                ...
                {!questionPoolAction ? (
                  <div className="d-grid gap-3">
                    <button className="btn btn-outline-success btn-lg" onClick={() => handleQuestionPoolAction('create')}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                    ...
                    <button className="btn btn-outline-primary btn-lg" onClick={() => handleQuestionPoolAction('import')}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                  ...
                  <form onSubmit={handleQuestionPoolSubmit}>
                    ...
                    <div className="d-flex gap-2">
                      <button type="button" className="btn btn-secondary flex-fill" onClick={() => setQuestionPoolAction('')} disabled={loading}>
                        Back
                      </button>
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content">
              <div className="modal-header border-0">
                ...
                <button type="button" className="btn-close" onClick={() => { setShowAddQuestionModal(false); setError(''); }} disabled={loading}></button>
              </div>
              <div className="modal-body">
                {error && <div className="alert alert-danger">{error}</div>}
                ...
                <form onSubmit={handleAddQuestion}>
                  ...
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      ...
                      <input type="text" className="form-control" placeholder="Option 1" value={newQuestion.option1} onChange={(e) => setNewQuestion({ ...newQuestion, option1: e.target.value })} required disabled={loading} />
                    </div>
                    <div className="col-md-6 mb-3">
                      <label className="form-label fw-medium">Option 2 *</label>
                      <input type="text" className="form-control" placeholder="Option 2" value={newQuestion.option2} onChange={(e) => setNewQuestion({ ...newQuestion, option2: e.target.value })} required disabled={loading} />
                    </div>
                    <div className="col-md-6 mb-3">
                      <label className="form-label fw-medium">Option 3 *</label>
                      <input type="text" className="form-control" placeholder="Option 3" value={newQuestion.option3} onChange={(e) => setNewQuestion({ ...newQuestion, option3: e.target.value })} required disabled={loading} />
                    </div>
                    <div className="col-md-6 mb-3">
                      <label className="form-label fw-medium">Option 4 *</label>
                      <input type="text" className="form-control" placeholder="Option 4" value={newQuestion.option4} onChange={(e) => setNewQuestion({ ...newQuestion, option4: e.target.value })} required disabled={loading} />
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-md-4 mb-3">
                      ...
                      <select className="form-select" value={newQuestion.correctAnswer} onChange={(e) => setNewQuestion({ ...newQuestion, correctAnswer: e.target.value })} required disabled={loading}>
                    ...
                    <div className="col-md-4 mb-3">
                      ...
                      <select className="form-select" value={newQuestion.difficulty} onChange={(e) => setNewQuestion({ ...newQuestion, difficulty: e.target.value })} required disabled={loading}>
                        <option value="easy">Easy</option>
                        <option value="medium">Medium</option>
                        <option value="hard">Hard</option>
                      </select>
                    ...
                    <div className="col-md-4 mb-3">
                      ...
                      <input type="number" className="form-control" min="1" value={newQuestion.marks} onChange={(e) => setNewQuestion({ ...newQuestion, marks: e.target.value })} required disabled={loading} />
                    </div>
                  </div>

                  <div className="d-flex gap-2">
                    <button type="button" className="btn btn-secondary flex-fill" onClick={() => setShowAddQuestionModal(false)} disabled={loading}>
                      Cancel
                    </button>
                    <button type="submit" className="btn btn-success flex-fill" disabled={loading}>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header border-0">
                ...
                <button type="button" className="btn-close" onClick={() => { setShowUploadCSVModal(false); setError(''); setCsvFile(null); setUploadProgress(0); }} disabled={loading}></button>
              </div>
              <div className="modal-body">
                {error && <div className="alert alert-danger">{error}</div>}
                ...
                <form onSubmit={handleCSVUpload}>
                  <div className="mb-3">
                    <label className="form-label fw-medium">Select Question Pool *</label>
                    <select className="form-select" value={selectedPoolForUpload} onChange={(e) => setSelectedPoolForUpload(e.target.value)} required disabled={loading}>
                      <option value="">Select Pool</option>
                      {questionPools.map((pool) => (
                        <option key={pool._id} value={pool._id}>
                          {pool.poolName}
                  ...
                  <div className="mb-3">
                    ...
                    <input type="file" className="form-control" accept=".csv" onChange={(e) => setCsvFile(e.target.files[0])} required disabled={loading} />
                  ...
                  <button type="button" className="btn btn-link btn-sm p-0 mb-3" onClick={downloadCSVTemplate}>
                  ...
                  <div className="d-flex gap-2">
                    <button type="button" className="btn btn-secondary flex-fill" onClick={() => setShowUploadCSVModal(false)} disabled={loading}>
                      Cancel
                    </button>
                    <button type="submit" className="btn btn-success flex-fill" disabled={loading}>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header border-0">
                ...
                <button type="button" className="btn-close" onClick={() => { setShowCourseModal(false); setError(''); }} disabled={loading}></button>
              </div>
              <div className="modal-body">
                {error && <div className="alert alert-danger">{error}</div>}
                ...
                <form onSubmit={handleCourseSubmit}>
                  <div className="mb-3">
                    ...
                    <input type="text" className="form-control" placeholder="Enter course name" value={courseData.courseName} onChange={(e) => setCourseData({ ...courseData, courseName: e.target.value })} required disabled={loading} />
                  </div>
                  <div className="mb-3">
                    <label className="form-label fw-medium">Description</label>
                    <textarea className="form-control" rows="3" placeholder="Enter course description" value={courseData.description} onChange={(e) => setCourseData({ ...courseData, description: e.target.value })} disabled={loading}></textarea>
                  </div>
                  <div className="mb-3">
                    <label className="form-label fw-medium">Duration (minutes) *</label>
                    <input type="number" className="form-control" placeholder="Enter duration" value={courseData.duration} onChange={(e) => setCourseData({ ...courseData, duration: e.target.value })} required min="1" disabled={loading} />
                  </div>
                  <div className="mb-3">
                    <label className="form-label fw-medium">Select Question Pool *</label>
                    <select className="form-select" value={courseData.questionPool} onChange={(e) => setCourseData({ ...courseData, questionPool: e.target.value })} required disabled={loading}>
                  ...
                  <div className="d-flex gap-2">
                    <button type="button" className="btn btn-secondary flex-fill" onClick={() => setShowCourseModal(false)} disabled={loading}>
                      Cancel
                    </button>
                    <button type="submit" className="btn btn-success flex-fill" disabled={loading || questionPools.length === 0}>
